          ...
          <div class="input-group-text">
            <template v-for="(day, i) in dayOfWeek" :key="i">
              <input class="form-check-input mt-0" type="checkbox" :id="'dayOfWeekInlineCheckbox'+day.value"
                     :value="editorData.daysOfWeek" :checked="editorData.daysOfWeek.includes(day.value)" @input="updateDaysOfWeek(day.value)">
...
export default {
  ...
  props: ['crawlScheduleCron', 'cronEditorData', 'activeTab', 'editorData'],
  data() {
    return {
      isValid: null,
      // activeTab: 'Weekly',
  ...
  methods: {
    ...
    updateDaysOfWeek(event) {
      let editorData = this.editorData
      editorData.daysOfWeek.push(event)
      this.$emit('updateCronEditorData', JSON.stringify({name: this.activeTab, editorData: editorData}))
    },
  }
}
