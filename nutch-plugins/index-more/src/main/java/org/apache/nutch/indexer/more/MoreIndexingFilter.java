...
//  private static Pattern charsetPatternHTML5 = Pattern.compile("<meta\\s+charset\\s*=\\s*[\"']?([a-z][_\\-0-9a-z]*)[^>]*>", Pattern.CASE_INSENSITIVE);
  ...
  public NutchDocument filter(NutchDocument doc, Parse parse, Text url,
    ...
    addCollectionID(doc, parse.getData(), url_s, datum);

    /*if (parse.getData().getMeta(Response.CONTENT_TYPE).contains("text/html")) {
      getRelevanceContent(doc, parse, url_s);
    }*/
  ...
  private NutchDocument addCollectionID(NutchDocument doc, ParseData data, String url, CrawlDatum datum) {
    ...
    if (collectionIDs != null) {
      ...
      doc.add("collectionID", fieldModifier);
    }
  ...
  /*private NutchDocument getRelevanceContent(NutchDocument doc, Parse parse, String url) {
  ...
  public void setConf(Configuration conf) {
    ...
    defaultCharEncoding = getConf().get("parser.character.encoding.default", "windows-1252");
    MAX_CONTENT_LENGTH = conf.getInt("indexer.max.content.length", -1);*/
  }
