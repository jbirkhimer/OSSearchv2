...
public class DummyIndexWriter implements IndexWriter {
  ...
  public void open(IndexWriterParams parameters) throws IOException {
    ...
    String collectionID = String.join("_", config.get("moreIndexingFilter.collectionIDs").split(" "));
    String dateSuffix = new SimpleDateFormat("yyyy-MM-dd_HH.mm.ss").format(new Date());
    path = parameters.get(DummyConstants.PATH, "./index_logs")+"/collectionID_["+collectionID+"]_("+dateSuffix+").txt";
    boolean path_mkdirs = new File(path).getParentFile().mkdirs();
    if (path == null) {
